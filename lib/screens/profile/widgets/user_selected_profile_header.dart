...
class UserSelectedProfilePageHeader extends StatefulWidget {
  ...
  const UserSelectedProfilePageHeader({Key? key, required this.userApp}) : super(key: key);
...
class _UserSelectedProfilePageHeaderState
  ...
  void _subscribeToUser(context, checkFollowing) async {
    ...
    await RepositoryProvider.of<UsersRepository>(context).updateUserFollowers(widget.userApp.id, userState!);
    await RepositoryProvider.of<UsersRepository>(context).updateUserFollowings(userState, widget.userApp.id!).then((value) {
      if (value == true) {
